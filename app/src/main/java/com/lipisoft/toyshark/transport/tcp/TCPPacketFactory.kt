...
object TCPPacketFactory {
    ...
    fun createSynAckPacketData(ip: IPv4Header, tcp: TCPHeader): Packet {
        ...
        tcpHeader.sourcePort = sourcePort
        tcpHeader.ackNumber = ackNumber // ack = received sequence + 1
        tcpHeader.sequenceNumber = seqNumber // initial sequence number generated by server
        ...
        tcpHeader.timeStampSender = Date().time.toInt()

        return Packet(
                ipHeader = ipHeader,
                transportHeader = tcpHeader,
                buffer = createPacketData(ipHeader, tcpHeader, null)
        )
    }
    ...
    private fun createPacketData(
        ...
        val tcpChecksum = PacketUtil.calculateTCPHeaderChecksum(
                data = buffer,
                offset = tcpStart,
                tcpLength = tcpBuffer.size + dataLength,
