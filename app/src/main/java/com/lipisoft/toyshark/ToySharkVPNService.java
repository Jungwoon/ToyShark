...
public class ToySharkVPNService extends VpnService implements Handler.Callback,
    ...
    private SocketDataPublisher socketDataPublisher;
    ...
    public void receive(byte[] packet) {
        if (pCapFileWriter != null) {
            try {
                pCapFileWriter.addPacket(packet, 0, packet.length, System.currentTimeMillis() * 1000000);
            } catch (IOException e) {
                Log.e(TAG, "pCapFileWriter.addPacket IOException :" + e.getMessage());
                e.printStackTrace();
            }
        } else {
    ...
    public void onDestroy() {
        ...
        try {
            ...
            Log.d(TAG, "fileDescriptor.close():" + e.getMessage());
            e.printStackTrace();
        }
    ...
    private void intializePcapFile() throws IOException {
        ...
        pCapFileWriter = new PCapFileWriter(pcapFile);
    }
    ...
    boolean startVpnService() {
        ...
        if (fileDescriptor != null) {
            Log.i(TAG, "VPN Established:interface = " + fileDescriptor.getFileDescriptor().toString());
            return true;
        } else {
    ...
    void startCapture() throws IOException {
        ...
        FileInputStream clientReader = new FileInputStream(fileDescriptor.getFileDescriptor());

        // Packets received need to be written to this output stream.
        FileOutputStream clientWriter = new FileOutputStream(fileDescriptor.getFileDescriptor());
        ...
        socketDataPublisher = new SocketDataPublisher();
        socketDataPublisher.subscribe(this);
        packetQueueThread = new Thread(socketDataPublisher);
